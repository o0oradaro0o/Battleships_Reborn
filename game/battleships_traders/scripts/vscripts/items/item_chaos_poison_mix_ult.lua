item_chaos_poison_mix_ult = class({})
LinkLuaModifier("modifier_item_chaos_poison_mix_ult_bow", "items/item_chaos_poison_mix_ult.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_chaos_poison_mix_ult_bow_2", "items/item_chaos_poison_mix_ult.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_chaos_poison_mix_ult_poison_debuff", "items/item_chaos_poison_mix_ult.lua", LUA_MODIFIER_MOTION_NONE)
...
----------------------------------------------------------------------
-- Primary Weapon: Ultimate Chaos (Enhanced random damage)
----------------------------------------------------------------------
...
function modifier_item_chaos_poison_mix_ult_bow:OnCreated(kv)
  if IsServer() then
    ...
    self.parent:AddNewModifier(self.parent, self.ability, "modifier_item_chaos_poison_mix_ult_bow_2", {})
...
function modifier_item_chaos_poison_mix_ult_bow:OnIntervalThink()
  if IsServer() then
    ...
    -- Chaos ultimate damage multiplier: random between -0.5 and 2.5
    local dmgMult = RandomFloat(-0.5, 2.5)
    local actual_damage = self.base_damage * dmgMult
    
    -- Select particle based on damage multiplier
    local particleName = "particles/basic_projectile/chaos_ult_projectile.vpcf"
    if dmgMult < 0.0 then
      particleName = "particles/basic_projectile/caulk_one_projectile.vpcf"
    elseif dmgMult < 0.5 then
      particleName = "particles/basic_projectile/chaos_ult_projectile_weak.vpcf"
    end
    ...
    local projectile = {
      ...
      ExtraData = {
        ModifierName = "modifier_item_chaos_poison_mix_ult_bow",
...
function modifier_item_chaos_poison_mix_ult_bow:OnRefresh(keys)
...
function modifier_item_chaos_poison_mix_ult_bow:OnDestroy()
  if IsServer() then
    self.parent:RemoveModifierByName("modifier_item_chaos_poison_mix_ult_bow_2")
  end
end

----------------------------------------------------------------------
-- Secondary Weapon: Ultimate Poison (4 projectiles with DoT)
----------------------------------------------------------------------

modifier_item_chaos_poison_mix_ult_bow_2 = class({})

function modifier_item_chaos_poison_mix_ult_bow_2:GetAbilityTextureName()
  local abilityName = self.ability:GetAbilityName()
  return abilityName
end

function modifier_item_chaos_poison_mix_ult_bow_2:GetAttributes()
  return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_item_chaos_poison_mix_ult_bow_2:OnCreated(kv)
  if IsServer() then
    self.ability = self:GetAbility()
    self.parent = self:GetParent()
...
function modifier_item_chaos_poison_mix_ult_bow_2:OnIntervalThink()
  if IsServer() then
    local enemies =
      FindUnitsInRadius(
      self.parent:GetTeam(),
    ...
    for i = 1, self.num_attacks do
      ...
      local projectile = {
        ...
        iSourceAttachment = DOTA_PROJECTILE_ATTACHMENT_HITLOCATION,
        ExtraData = {
          ModifierName = "modifier_item_chaos_poison_mix_ult_bow_2",
...
function modifier_item_chaos_poison_mix_ult_bow_2:OnRefresh(keys)
  if not IsServer() then
    return
  end
  ...
  -- Apply poison DoT
  keys.target:AddNewModifier(keys.caster, self:GetAbility(), "modifier_item_chaos_poison_mix_ult_poison_debuff", {duration = self.psn_dur})
end
...
modifier_item_chaos_poison_mix_ult_poison_debuff = class({})

function modifier_item_chaos_poison_mix_ult_poison_debuff:IsDebuff()
  return true
end

function modifier_item_chaos_poison_mix_ult_poison_debuff:IsHidden()
  return false
end

function modifier_item_chaos_poison_mix_ult_poison_debuff:GetAttributes()
  return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_item_chaos_poison_mix_ult_poison_debuff:GetEffectName()
  return "particles/basic_projectile/status_effect_poison.vpcf"
end

function modifier_item_chaos_poison_mix_ult_poison_debuff:OnCreated()
  if IsServer() then
    self:StartIntervalThink(0.5)
  end
end

function modifier_item_chaos_poison_mix_ult_poison_debuff:OnIntervalThink()
  if IsServer() then
    local damageTable = {
