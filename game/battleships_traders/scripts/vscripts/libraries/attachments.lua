...
function Attachments:start()
  ...
  --VicFrank I had to add this to make it work
  self.initialized = true
...
function Attachments:AttachProp(unit, attachPoint, model, scale, properties)
    ...
    local defaultModelSize = 150 --VicFrank: This scales all models to be this size (prior to other scaling)
    ...
    if model.GetName and IsValidEntity(model) then
      ...
      prop:SetModelScale(scale * unit:GetModelScale() * (defaultModelSize / (((prop:GetBoundingMaxs() - prop:GetBoundingMins()):Length2D()))))
    end
